...
template <typename scalar_type>
std::vector<scalar_type> gauss_partial_parallel(const matrix<scalar_type>& a0,
    const std::vector<scalar_type>& b0) {
    size_t n = a0.rows();
    assert(a0.columns() == n);
    assert(b0.size() == n);
...
int main() {
    ...
    std::cout << "time taken in seconds: " << time_taken << "s\n";
    ...
    std::vector<double> parallel(gauss_partial_parallel(a, b));
    auto time_taken_p = omp_get_wtime() - start_time_p;
    std::cout << "Parallel time taken in seconds: " << time_taken_p << "s\n";       
    for (size_t i = 0; i < parallel.size(); ++i) {
        assert(std::abs(x[i] - parallel[i]) <= epsilon);
        std::cout << parallel[i] << '\n';
    }
