...
public:
    ...
    //    SubMatrix m(*this); 
    //    int sign = 1;
    //    /*std::promise<double> detPromise;
    //    std::future<double> detFuture = detPromise.get_future();*/
    //    /*std::vector<std::thread> threads;*/

    //    //protect the thread
    //    /*std::mutex mutex_lock;
    //    const std::lock_guard<std::mutex> lock(mutex_lock);
    //    threads.push_back(std::thread(thread_det(m, sign, det), ));*/
    ...
    //private:
    //    void thread_det(std::promise<double>&& detPromise, SubMatrix &m, int &sign, double &det) {
    //        detPromise.set_value(m.det());
    //    }
};
...
std::vector<double> solveCramer(std::promise<double> && p, const std::vector<std::vector<double>>& equations) {
...
int main() {
    ...
    start_time = omp_get_wtime();
    std::promise<double> solutionPromise;
    std::future<double> solutionFuture = solutionPromise.get_future();
    std::thread t(&solveCramer, &equations);
    t.join();
    double solution = solutionFuture.get();
